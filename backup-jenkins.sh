...
function backup_jobs() {
    ...
    if [ -d "${run_in_path}" ]; then
        ...
        find . -maxdepth 1 -type d | while read -r job_name; do
            ...
            elif [ -f "${JENKINS_HOME}/jobs/${rel_depth}/${job_name}/config.xml" ] && [ "$(grep -c "org.jenkinsci.plugins.workflow.multibranch.WorkflowMultiBranchProject" "${JENKINS_HOME}/jobs/${rel_depth}/${job_name}/config.xml")" -ge 1 ]; then
                #echo "Multibranch! $JENKINS_HOME/jobs/$rel_depth/$job_name/branches"
                backup_jobs "${JENKINS_HOME}/jobs/${rel_depth}/${job_name}/branches"
            else
                true
